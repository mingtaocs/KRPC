...
void send_request(int thread_id, std::atomic<int> &success_count, std::atomic<int> &fail_count) {
    // 创建一个 FiendServiceRpc_Stub 对象，用于调用远程的 RPC 方法
    Kfriend::FiendServiceRpc_Stub stub(new KrpcChannel(false));

    // 设置 RPC 方法的请求参数
    Kfriend::GetFriendsListRequest request;
    ...
    if (controller.Failed()) {  // 如果调用失败
        ...
        if (0 == response.result().errcode()) {  // 检查响应中的错误码
            ...
            std::cout << "rpc GetFriendsList response response error : " << response.result().errmsg() << std::endl;  // 打印错误信息
            fail_count++;  // 失败计数加 1
        }
    }
}
